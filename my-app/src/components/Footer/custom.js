...
$(document).ready(function () {
  function footer() {
    var windowWidth = $(window).width();
    if (windowWidth < 768) {
      let footerLinkWraper = $(".footer-links-wrapper h3");
      footerLinkWraper.on("click",function(){ //off("click")?
        // footerLinkWraper.css("position", "relative");
        // footerLinkWraper.toggleClass("expanded");// this will manuplate every footerLinkWraper(h3)
        // let allLinks=$(".footer-links-wrapper")
        // allLinks.addclass("offset1")
        $(this).toggleClass("expanded");//this will refer us to the clicked h3
        let footerUl = $(this).next("ul");
        // let footerList = $(".footer-links-wrapper ul");
